...
namespace CGPG
{
    internal class Renderer : GameWindow
    {
        ...
        protected override void OnRenderFrame(FrameEventArgs args)
        {
            ...
            double time = 0.0 * (float)Math.PI / 180.0f;
            Matrix4 model = Matrix4.CreateRotationY((float)time) * Matrix4.CreateRotationX((float)time * 0.5f);
            Matrix4 view = Matrix4.CreateTranslation(0f, 0f, -7f);
            ...
            _mvp = custom * model * view * projection;

            int mvpLoc = GL.GetUniformLocation(_shaderProgram, "uMVP");
            GL.UniformMatrix4(mvpLoc, false, ref _mvp);
